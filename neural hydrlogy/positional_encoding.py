class _PositionalEncoding(nn.Module):
    ...
    '''
    Comment from Kangmin: the Vanilla Positional Encoding can not make fully use of the order information
    For example: input A: (365 days) from July 1st to June 30th, input B: (365 days) from January 1st to December 31th
    The first position July 1st and January 1st will get the same positional encoding
    Therefore, I'd like to use time features encoding
    '''
    ...
    def __init__(self, embedding_dim, position_type, dropout, max_len=5000):
        ...
        pe = torch.zeros(max_len, int(np.ceil(embedding_dim / 2) * 2)) # zero matrix [max_len, embedding_dim]
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1) #  0~max_len - 1 matrix [max_len, 1] position index
